...
import MacroOverviewHeader from "@/components/MacroOverviewHeader";
import TransmissionFlow from "@/components/TransmissionFlow";
import CountryExposureGrid from "@/components/CountryExposureGrid";
import DecisionPriorityPanel from "@/components/DecisionPriorityPanel";
...
export default function HomePage() {
  ...
  const detailLabels: Record<Language, Record<DetailView, string>> = {
    en: { dashboard: "Macro Overview", banking: "Banking Stress", insurance: "Insurance Stress", fintech: "Fintech Stress", decisions: "Decision Priorities" },
    ar: { dashboard: "نظرة عامة", banking: "ضغط القطاع البنكي", insurance: "ضغط التأمين", fintech: "ضغط الفنتك", decisions: "أولويات القرار" },
  };
  ...
  return (
    <div className="min-h-screen bg-io-bg" dir={isAr ? "rtl" : "ltr"}>
      ...
      {/* Macro-First Dashboard */}
      {result && detailView === "dashboard" && (
        <div className="min-h-screen bg-io-bg" dir={isAr ? "rtl" : "ltr"}>
          {/* SECTION 1: Stress Context — Macro Overview */}
          <MacroOverviewHeader data={result} lang={lang} />

          {/* SECTION 2: Transmission Mechanics */}
          <TransmissionFlow data={result} lang={lang} />

          {/* SECTION 3: Sector Exposure Detail */}
          <CountryExposureGrid data={result} lang={lang} />

          {/* SECTION 4: Decision Priorities (above graph!) */}
          <DecisionPriorityPanel data={result} lang={lang} onViewAll={() => setDetailView("decisions")} />

          {/* SECTION 5: Trust & Confidence */}
          <TrustPanel data={result} lang={lang} />

          {/* SECTION 6: Operational Detail (graph is NOT primary) */}
          <OperationalPanel data={result} lang={lang} onNavigate={(view: string) => setDetailView(view as DetailView)} />
        </div>
      )}
