...
RELATIONSHIP_TYPES = [
    ...
    # Signal ingestion
    "TRIGGERS",          # (Signal)-[:TRIGGERS]->(Event)
    "CORRELATES_WITH",   # (Signal)-[:CORRELATES_WITH]->(Signal)
    "IMPACTS",           # (Event)-[:IMPACTS]->(Entity)
]
...
INDEXES: list[str] = [
    ...
    "CREATE INDEX signal_confidence_idx IF NOT EXISTS FOR (s:Signal) ON (s.confidence)",
    "CREATE INDEX signal_event_time_idx IF NOT EXISTS FOR (s:Signal) ON (s.event_time)",
]
